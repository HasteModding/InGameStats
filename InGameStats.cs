...
public class InGameStats : MonoBehaviour {
    ...
    private string GetStatValue(StatType stat) {
        ...
        return stat switch {
            ...
            StatType.UpcomingLevel => RunHandler.RunData.isEndless ? "Endless" : InGameStatsUtils.GetUpcomingLevelStats(),
            StatType.ItemUnlockProgression => InGameStatsUtils.GetItemUnlockProgression(itemUnlockProgressionMode),
            StatType.Shard => RunHandler.RunData.isEndless ? "Endless" : (RunHandler.RunData.shardID + 1).ToString(),
